...
const resetPassword = async (req : Request,res : Response) :Promise<Response<any, Record<string, {message: string} | {message: string, token: string}>>> => {
	...
	if(!e_mail)
		return res.status(422).json({error : "Email Field is Empty!"});
	if(!user_type)
		return res.status(422).json({error : "User Type Field is Empty!"});
	if(!new_password)
		return res.status(422).json({error : "New Password Field is Empty!"});

	const result = await EcSuppliers.update({ password : new_password },{where : {e_mail : e_mail}});
